...
function Levantamento() {
  ...
  const obterEstadoInicialFigurino = (estadoNome, estadoId) => {
  ...
  const toggleVerificado = (idItem) => {
    setChecklist((prev) => prev.map((item) => item.id === idItem ? { ...item, verificado: !item.verificado } : item));
  };

  const atualizarObservacoes = (idItem, observacoes) => {
    setChecklist((prev) => prev.map((item) => item.id === idItem ? { ...item, observacoes } : item));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (acessoriosNaoSelecionados.length > 0) {
      const continuar = window.confirm(
        `Existem acessorios nao selecionados: ${acessoriosNaoSelecionados.map((item) => item.nome).join(", ")}.\n\n` +
        "Se continuar, estes acessorios ficam registados como nao entregues no levantamento e nao serao esperados na devolucao. Pretende continuar?"
      );
      if (!continuar) return;
    }
    ...
    if (!reserva || !linhaReserva) return;
  ...
  if (carregando) {
    return <div className="text-center py-12"><p className="text-gray-600">A carregar reserva...</p></div>;
  }
  if (!figurino) {
    return <div className="text-center py-12">
      <p className="text-gray-600">Reserva nao encontrada</p>
      <Link to="/reservas" className="text-purple-600 hover:text-purple-700 mt-4 inline-block">Voltar as reservas</Link>
    </div>;
  }
  ...
  return <div className="space-y-6">
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {reserva && reserva.linhas.filter((l) => l.estado?.toUpperCase() !== "CANCELADA").length > 1 && <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        >
          {reserva.linhas.filter((linha) => linha.estado?.toUpperCase() !== "CANCELADA").map((linha) => <option key={linha.id} value={linha.id}>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><p className="text-sm text-gray-600">Nome</p><p className="font-medium text-gray-900">{figurino.nome}</p></div>
          <div><p className="text-sm text-gray-600">Tamanho</p><p className="font-medium text-gray-900">{figurino.tamanho}</p></div>
          <div className="md:col-span-2"><p className="text-sm text-gray-600">Descricao</p><p className="font-medium text-gray-900">{figurino.descricao}</p></div>
          <div><p className="text-sm text-gray-600">Localizacao</p><p className="font-medium text-gray-900">{figurino.localizacao}</p></div>
          <div><p className="text-sm text-gray-600">Periodo</p><p className="font-medium text-gray-900">{linhaReserva && new Date(linhaReserva.data_inicio).toLocaleDateString("pt-PT")} - {linhaReserva && new Date(linhaReserva.data_fim).toLocaleDateString("pt-PT")}</p></div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            >
              {estadosCondicao.length === 0 && <option value="">Sem estados disponiveis</option>}
              {estadosCondicao.map((estado) => <option key={estado.id} value={estado.id}>{estado.nome}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Observacoes Gerais</label>
            <textarea
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="space-y-4">
          {checklist.map((item) => <div key={item.id} className="border rounded-lg p-4">
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={item.verificado} onChange={() => toggleVerificado(item.id)} className="w-5 h-5 text-purple-600 rounded focus:ring-purple-500" />
              <div className="flex-1"><p className="font-medium text-gray-900">{item.nome}</p></div>
            </label>
            {item.verificado && <div className="mt-3">
              <input
                type="text"
                value={item.observacoes}
                onChange={(e) => atualizarObservacoes(item.id, e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                placeholder="Observacoes sobre este acessorio (opcional)"
              />
            </div>}
          </div>)}
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Assinatura do Funcionario ({utilizadorAtual?.nome})</label>
            <div className="border-2 border-gray-300 rounded-lg overflow-hidden">
              <SignatureCanvas ref={assinaturaFuncionarioRef} canvasProps={{ className: "w-full h-40 bg-gray-50" }} />
            </div>
            <button type="button" onClick={() => limparAssinatura("funcionario")} className="text-sm text-purple-600 hover:text-purple-700 mt-2">Limpar Assinatura</button>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Assinatura do Cliente ({reserva?.utilizador.nome})</label>
            <div className="border-2 border-gray-300 rounded-lg overflow-hidden">
              <SignatureCanvas ref={assinaturaClienteRef} canvasProps={{ className: "w-full h-40 bg-gray-50" }} />
            </div>
            <button type="button" onClick={() => limparAssinatura("cliente")} className="text-sm text-purple-600 hover:text-purple-700 mt-2">Limpar Assinatura</button>
          </div>
        </div>
      </div>

      <div className="flex items-center justify-end gap-4">
        <Link to="/reservas" className="px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">Cancelar</Link>
        <button type="submit" className="flex items-center gap-2 px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
