...
function CriarFigurino() {
  ...
  const [estadosCondicao, setEstadosCondicao] = useState([]);
  ...
  const toggleAcessorio = (idAcessorio) => {
  ...
  const adicionarNovoAcessorio = async () => {
    ...
    if (existente) {
      ...
      toast.info(`O acess\xF3rio "${existente.nome}" j\xE1 existia e foi selecionado.`);
      return;
    }
    ...
    try {
      ...
      setNovoAcessorio("");
      setMostrarNovoAcessorio(false);
      toast.success(`Acess\xF3rio "${novo.nome}" criado e selecionado.`);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/figurinos"), 1e3);
    } catch (err) {
      toast.error(err.message || "Erro de liga\xE7\xE3o ao servidor");
    } finally {
      setAGuardar(false);
    }
  };
    ...
    /* Upload de Imagens */
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-4">
            {imagens.map((imagem, index) => <div key={index} className="relative group aspect-square">
                <img
    src={imagem}
  ...
  >
                <option value="">Selecione...</option>
                {estadosCondicao.map((e) => <option key={e.id} value={e.id}>{e.nome}</option>)}
              </select>
            </div>
  ...
  >
          ...
          {mostrarNovoAcessorio && <div className="mb-4 flex gap-2">
              <input
    type="text"
    value={novoAcessorio}
  ...
  >
          ...
          {acessorios.length === 0 ? <p className="text-sm text-gray-400 italic">Nenhum acessório disponível</p> : <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {acessorios.map((acessorio) => <label
    key={acessorio.id}
    className={`flex items-center gap-2 p-3 border-2 rounded-lg cursor-pointer transition-colors ${acessoriosSelecionados.includes(acessorio.id) ? "border-purple-500 bg-purple-50" : "border-gray-200 hover:border-purple-300"}`}
  >
                  <input
    type="checkbox"
  ...
  />
          ...
          {acessoriosSelecionados.length > 0 && <p className="text-sm text-gray-600 mt-3">
