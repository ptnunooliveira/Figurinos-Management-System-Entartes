...
import { getReservas, getMinhasReservas, cancelarReserva, atualizarEstadoLinhaReserva } from "../lib/services";
...
function Reservas() {
  ...
  const handleAprovarLinha = async (idReserva, idLinha) => {
    try {
  ...
  const handleRecusarLinha = async (idReserva, idLinha) => {
  ...
  const derivarEstadoReserva = (reserva) => {
  ...
  const sortDesc = (arr) => [...arr].sort((a, b) => (b.id ?? 0) - (a.id ?? 0));
  ...
  const reservasFiltradas = reservasExibir.filter((reserva) => {
    ...
    const matchTermo = (reserva.id?.toString() || "").includes(termoPesquisa) || (reserva.utilizador?.nome || "").toLowerCase().includes(termoPesquisa.toLowerCase()) || (reserva.utilizador?.email || "").toLowerCase().includes(termoPesquisa.toLowerCase()) || (reserva.linhas || []).some(
      (linha) => linha.anuncio?.figurino?.nome?.toLowerCase().includes(termoPesquisa.toLowerCase()) || linha.anuncio?.figurino?.categoria?.toLowerCase().includes(termoPesquisa.toLowerCase()) || linha.anuncio?.figurino?.tamanho?.toLowerCase().includes(termoPesquisa.toLowerCase())
    );
  ...
  return <div className="space-y-3">
    ...
    /* Cabeçalho */
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900 mb-0.5">
          {isStaff ? "Gest\xE3o de Reservas" : "Minhas Reservas"}
        </h1>
    ...
    className={`px-5 py-1.5 rounded-lg text-sm transition-colors ${abaAtiva === "ativas" ? "bg-gradient-to-r from-fig-purple to-fig-magenta text-white" : "text-gray-600 hover:bg-gray-100"}`}
  >
          Ativas ({reservasAtivas.length})
        </button>
        <button
    onClick={() => setAbaAtiva("historico")}
    className={`px-5 py-1.5 rounded-lg text-sm transition-colors ${abaAtiva === "historico" ? "bg-gradient-to-r from-fig-purple to-fig-magenta text-white" : "text-gray-600 hover:bg-gray-100"}`}
  >
  ...
  >
    ...
    return <div key={reserva.id} className="bg-white rounded-xl shadow-sm overflow-hidden flex">
                {
      ...
      return <div key={linha.id} className="flex items-center gap-3 px-3 py-2">
                        {
        /* Imagem */
      }
      ...
      >
                          ...
                          {eStaff && eAtiva && estadoLinha === "CONFIRMADA" && <Link
      ...
      >
                          ...
                          {eStaff && eAtiva && estadoLinha === "EM CURSO" && <Link
      ...
      >
                          ...
                          {!eStaff && eAtiva && estadoLinha === "PENDENTE" && <button
      ...
      >
            ...
            <h3 className="text-base font-medium text-gray-900 mb-1">
              {abaAtiva === "ativas" ? "Nenhuma reserva ativa" : "Nenhuma reserva no hist\xF3rico"}
            </h3>
            <p className="text-sm text-gray-600 mb-4">
              {abaAtiva === "ativas" ? isStaff ? "N\xE3o h\xE1 reservas ativas no momento." : "Ainda n\xE3o tem reservas ativas. Explore os an\xFAncios da escola!" : "Ainda n\xE3o h\xE1 hist\xF3rico de reservas."}
            </p>
            {abaAtiva === "ativas" && !isStaff && <Link
    to="/anuncios-escola"
