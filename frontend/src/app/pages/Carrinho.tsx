...
export function Carrinho() {
  ...
  const handleFinalizarReserva = async () => {
    ...
    setSubmetendo(true);
    try {
      // Formata os itens do carrinho para o formato que a API espera
      ...
      // A função criarReserva é importada dos serviços e lida com a chamada à API.
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        {items.length === 0 ? (
          ...
          <div className="space-y-4">
            {items.map((item) => (
              <div key={item.cartItemId} className="flex items-center justify-between border-b pb-4 last:border-0 last:pb-0">
                <div className="flex items-start gap-4">
                  {item.imagem_url ? (
                    <img src={item.imagem_url} alt={item.figurino_nome} className="w-16 h-16 object-cover rounded-lg border" />
                  ) : (
