...
function Faturacao() {
  ...
  const [selecionados, setSelecionados] = useState(/* @__PURE__ */ new Set());
  ...
  const [filtroExportado, setFiltroExportado] = useState("nao");
  ...
  const movimentosFiltrados = contaCorrente.filter((mov) => {
    ...
    if (dataMovInicio || dataMovFim) {
      ...
      const inicio = dataMovInicio ? new Date(dataMovInicio) : /* @__PURE__ */ new Date(0);
      const fim = dataMovFim ? new Date(dataMovFim) : /* @__PURE__ */ new Date();
    ...
    if (dataExpInicio || dataExpFim) {
      ...
      const inicio = dataExpInicio ? new Date(dataExpInicio) : /* @__PURE__ */ new Date(0);
      const fim = dataExpFim ? new Date(dataExpFim) : /* @__PURE__ */ new Date();
      if (dataExp < inicio || dataExp > fim) return false;
    }
    if (filtroAluno && mov.nome_aluno !== filtroAluno) return false;
    if (filtroTexto) {
      const texto = filtroTexto.toLowerCase();
      if (!mov.descricao.toLowerCase().includes(texto) && !mov.tipo_movimento.toLowerCase().includes(texto) && !mov.nome_aluno.toLowerCase().includes(texto)) return false;
    }
    return true;
  }).sort((a, b) => {
  ...
  const todosSeleccionados = movimentosFiltrados.length > 0 && movimentosFiltrados.every((m) => selecionados.has(m.id));
  ...
  />
    ...
    onChange={(e) => setFiltroExportado(e.target.value)}
    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
  >
  ...
  >
                ...
                {alunosUnicos.map((nome) => <option key={nome} value={nome}>{nome}</option>)}
              </select>
            </div>}
    ...
    /* Resumo */
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          {movimentosSelecionados.length > 0 && <p className="text-sm text-purple-600 mt-1">{movimentosSelecionados.length} selecionado(s)</p>}
        </div>
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center gap-3 mb-2">
  ...
  >
            ...
            {movimentosSelecionados.length > 0 ? `Exportar (${movimentosSelecionados.length})` : "Exportar"}
          </button>
        </div>
      </div>
  ...
  />
                ...
                {isFuncionario && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    Aluno
                  </th>}
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Tipo
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {movimentosFiltrados.map((mov) => <tr
    key={mov.id}
  ...
  />
                  ...
                  {isFuncionario && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {mov.nome_aluno || "\u2014"}
                    </td>}
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs rounded font-medium ${mov.tipo_movimento === "Ocorr\xEAncia" ? "bg-orange-100 text-orange-700" : mov.tipo_movimento === "Reserva" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"}`}>
                      {mov.tipo_movimento || "\u2014"}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    {mov.exportado_faturacao ? <span className="text-green-600 text-xs font-medium">✓ Sim</span> : <span className="text-gray-400 text-xs">Não</span>}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                    {mov.data_exportacao ? new Date(mov.data_exportacao).toLocaleDateString("pt-PT") : "\u2014"}
                  </td>
                </tr>)}
            </tbody>
          </table>
        </div>
        {movimentosFiltrados.length === 0 && <div className="text-center py-12">
