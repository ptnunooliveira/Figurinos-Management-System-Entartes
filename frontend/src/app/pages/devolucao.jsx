...
const obterDadosObservacoesChecklist = (observacoes) => {
...
function Devolucao() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!reserva || !id || !linhaReserva) return;
    ...
    if (acessoriosEmFalta.length > 0) {
      const continuar = window.confirm(
        `Existem acessorios esperados que nao foram selecionados na devolucao: ${acessoriosEmFalta.map((item) => item.nome).join(", ")}.\n\n` +
        "Se continuar, sera gerada uma ocorrencia por acessorios em falta. Pretende continuar?"
      );
      if (!continuar) return;
    }
    ...
    try {
      ...
      if (ocorrenciaGerada) {
        ...
        toast.success("Devolucao registada com ocorrencia associada.");
      } else {
  ...
  return <div className="space-y-6">
    ...
    {ocorrenciaAlerta && <div className="bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg">
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="font-semibold text-orange-900">Devolucao registada com ocorrencia</p>
          <p className="text-sm text-orange-800 mt-1">Ocorrencia <strong>#{ocorrenciaAlerta.id}</strong> gerada automaticamente.</p>
          <button onClick={() => navigate("/reservas")} className="mt-3 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm transition-colors">Voltar as Reservas</button>
        </div>
      </div>
    </div>}
    ...
    {!ocorrenciaAlerta && <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="px-4 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-800 font-medium">{estadoInicialNome}</div>
        ...
        {idEstadoFigurinoSel != null && idEstadoLevantamento != null && idEstadoFigurinoSel > idEstadoLevantamento && <div className="bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg mt-6">
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-orange-900">O estado selecionado e pior do que o registado no levantamento. Ao gravar, sera criada automaticamente uma <strong>ocorrencia</strong> para analise.</p>
          </div>
        </div>}

        <div className="mt-6">
          <label className="block text-sm font-medium text-gray-700 mb-2">Observacoes do Levantamento</label>
          <div className="mb-4 min-h-11 rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm text-gray-800 whitespace-pre-wrap">
            {observacoesLevantamento || "Sem observacoes registadas no levantamento."}
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-sm text-gray-600 mb-4">Apenas aparecem os acessorios registados como entregues no levantamento.</p>
