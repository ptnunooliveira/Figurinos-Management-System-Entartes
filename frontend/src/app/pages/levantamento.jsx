...
function Levantamento() {
  ...
  const toggleVerificado = (id2) => {
    setChecklist(
      (prev) => prev.map(
        (item) => item.id === id2 ? { ...item, verificado: !item.verificado } : item
      )
    );
  };
  const atualizarObservacoes = (id2, observacoes) => {
    setChecklist(
      (prev) => prev.map(
        (item) => item.id === id2 ? { ...item, observacoes } : item
      )
    );
  };
